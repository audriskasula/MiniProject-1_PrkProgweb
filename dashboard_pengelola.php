...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id > 0) {
        ...
        if ($stmt->execute()) $success = "Kampanye berhasil diupdate.";
        else $error = "Gagal mengupdate kampanye.";
    } else {
        // Tambah baru
        if ($gambar == "") $gambar = "uploads/default.jpg"; // fallback
        ...
        if ($stmt->execute()) $success = "Kampanye baru berhasil ditambahkan.";
        else $error = "Gagal menambahkan kampanye.";
    }
}
...
<body>
    ...
    <div class="container" style="max-width: 1000px;">
        ...
        <div class="glass-card" style="margin-bottom: 3rem;">
            ...
            <form action="dashboard_pengelola.php" method="POST" enctype="multipart/form-data" style="margin-top: 1rem;">
                <input type="hidden" name="id" value="<?php echo $edit_data ? $edit_data['id'] : ''; ?>">
                
                <div class="form-layout">
                    <div class="form-group full-width">
                        <label>Judul Kampanye</label>
                        <input type="text" name="judul_kampanye" class="form-control" required value="<?php echo $edit_data ? htmlspecialchars($edit_data['judul_kampanye']) : ''; ?>">
                    </div>
                    
                    <div class="form-group">
                        <label>Kategori</label>
                        <select name="kategori" class="form-control" required>
                            <option value="Bencana Alam" <?php if($edit_data && $edit_data['kategori'] == 'Bencana Alam') echo 'selected'; ?>>Bencana Alam</option>
                            <option value="Pendidikan" <?php if($edit_data && $edit_data['kategori'] == 'Pendidikan') echo 'selected'; ?>>Pendidikan</option>
                            <option value="Kesehatan" <?php if($edit_data && $edit_data['kategori'] == 'Kesehatan') echo 'selected'; ?>>Kesehatan</option>
                            <option value="Lingkungan" <?php if($edit_data && $edit_data['kategori'] == 'Lingkungan') echo 'selected'; ?>>Lingkungan</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label>Lokasi</label>
                        <input type="text" name="lokasi" class="form-control" required value="<?php echo $edit_data ? htmlspecialchars($edit_data['lokasi']) : ''; ?>">
                    </div>

                    <div class="form-group full-width">
                        <label>Deskripsi</label>
                        <textarea name="deskripsi" class="form-control" rows="4" required><?php echo $edit_data ? htmlspecialchars($edit_data['deskripsi']) : ''; ?></textarea>
                    </div>

                    <div class="form-group">
                        <label>Target Dana (Rp)</label>
                        <input type="number" name="target_dana" class="form-control" required value="<?php echo $edit_data ? (int)$edit_data['target_dana'] : ''; ?>">
                    </div>

                    <div class="form-group">
                        <label>Batas Waktu</label>
                        <input type="date" name="batas_waktu" class="form-control" required value="<?php echo $edit_data ? $edit_data['batas_waktu'] : ''; ?>">
                    </div>

                    <div class="form-group">
                        <label>Rekening Donasi Tujuan</label>
                        <input type="text" name="rekening_donasi" class="form-control" required value="<?php echo $edit_data ? htmlspecialchars($edit_data['rekening_donasi']) : ''; ?>">
                    </div>
                ...
                <div style="margin-top: 1rem;">
                    <button type="submit" class="btn-primary"><?php echo $edit_data ? 'Simpan Perubahan' : 'Tambah Kampanye'; ?></button>
                    <?php if($edit_data): ?>
                        <a href="dashboard_pengelola.php" class="btn-primary" style="background: var(--text-muted); border-color: var(--text-muted);">Batal</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead style="background: rgba(255,255,255,0.05);">
                    <tr>
                        ...
                        <th style="padding: 15px; text-align: left; border-bottom: 1px solid var(--border);">Terkumpul</th>
                        <th style="padding: 15px; text-align: left; border-bottom: 1px solid var(--border);">Batas Waktu</th>
                ...
                <tbody>
                    <?php if (count($kampanyes) > 0): ?>
                        ...
                        <tr>
                            <td style="padding: 15px; border-bottom: 1px solid var(--border);"><?php echo htmlspecialchars($k['judul_kampanye']); ?></td>
                            <td style="padding: 15px; border-bottom: 1px solid var(--border);">Rp <?php echo number_format($k['target_dana'], 0, ',', '.'); ?></td>
                            <td style="padding: 15px; border-bottom: 1px solid var(--border);">Rp <?php echo number_format($k['dana_terkumpul'], 0, ',', '.'); ?></td>
                            <td style="padding: 15px; border-bottom: 1px solid var(--border);"><?php echo $k['batas_waktu']; ?></td>
                            <td style="padding: 15px; border-bottom: 1px solid var(--border); text-align: center;">
                                <a href="dashboard_pengelola.php?edit=<?php echo $k['id']; ?>" style="color: var(--primary); margin-right: 10px;">Edit</a>
                                <a href="dashboard_pengelola.php?delete=<?php echo $k['id']; ?>" style="color: var(--error);" onclick="return confirm('Yakin hapus kampanye ini?');">Hapus</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
